...
```{r, cache.comments=TRUE,comment=F,message=F, comment = F,error=FALSE,echo=T}
...
# MOD2 : Analyse spatiale / Régression
...
Soit l'ensemble des ventes de **maison** effectuées dans un rayon de 30-50 km autour d'une agglomération. Existe-t-il une relation entre (Y) le prix de vente mesuré en € par m2 de surface habitable et (X) la distance au point central de l'agglomération ? 

N.B. Nous ne travaillons que sur les maisons car les appartements sont trop peu nombreux en dehors du centre de l'aire urbaine.
...
On décide ici de retenir le modèle 3 (logarithmique) qui fournit le meilleur ajustement et de stocker les valeurs estimées et résiduelles des prix au m2.

```{r}
mod<-mod3
map_dvf$prixm2_estim <- mod$fitted.values
map_dvf$prixm2_resid <- map_dvf$prixm2-map_dvf$prixm2_estim
```
...
```{r}
mybreaks <-c( -500, -350, -250,-150, -50,50, 150,250, 350, 500)
mypal <- brewer.pal(n = 9, name="RdBu")
par(mfrow = c(1,1))
mf_map(map_com, 
       type = "choro",
       var="prixm2_resid",
...
## Zoom sur les communes à fort résidu

### Cesson-Sévigné 

```{r, echo=F}
knitr::include_graphics("figures/cesson.jpg")
```
> Source : [Les Echos, Aoüt 2018](https://www.lesechos.fr/2018/08/immobilier-cesson-sevigne-miser-sur-la-banlieue-chic-de-rennes-998217)

### St Jacques de la Lande

```{r, echo=F}
knitr::include_graphics("figures/lajanais.jpg")
```

> Source : [France 3 Bretagne](https://france3-regions.francetvinfo.fr/bretagne/ille-et-vilaine/rennes/la-janais-vue-d-ailleurs-226977.html)
